...
The friendly entrypoint is `./install.sh`; it presents an interface-first terminal menu, prints a run id plus planned flow, emits `[FLOW]` progress markers for wrapper phases, and wraps `./bootstrap.sh`, which remains the deterministic engine. The flow is intentionally staged so a fresh machine can start with one command, choose a safe dry-run/plan/apply action in the interface, see what will run, and then graduate to real install tiers.
...
When the menu action is preview/dry-run, the wrapper calls the engine in dry-run mode. Dry-run mode sets `BOOTSTRAP_DRY_RUN=1`; install scripts print the commands they would run rather than mutating the host. The wrapper trace is intentionally screen-only by default: it gives run-level orientation without creating persistent runtime logs in the repo.
